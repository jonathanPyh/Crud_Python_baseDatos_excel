...
from Connexion_BsDatos import Persona_datos
...
class Login(Frame):
    ...
    def widgets(self):
        ...
        Label(self.frame_uno, text='Correo', fg='black',bg='white',font=('Rockewll',13,'bold')).grid(column=0,row=3,pady=5)
        Label(self.frame_uno, text='Telefono', fg='black',bg='white',font=('Rockewll',13,'bold')).grid(column=0,row=4,pady=5)
        ...
        Button(self.frame_uno , text='AGREGAR DATOS', font=('Arial', 9,'bold'), bg='deep sky blue', width=20, bd=3, command=self.agregar_datos).grid(column=2, row=2, pady=5, padx=5)
        ...
        Button(self.frame_uno, text='AGREGAR A EXCEL', font=('Arial', 9, 'bold'), bg='deep sky blue', width=20, bd=3, command=self.guardar_datos).grid(column=2,row=5,pady=5, padx=5)

        estilo_tabla = ttk.Style()
        estilo_tabla.configure("Treeview", font=('Helvetica',10, 'bold'), foreground ='black', background='white')
        estilo_tabla.map('Treeview',background=[('selected', 'deep sky blue')], foregraund=[('selected', 'black')])
        ...
        self.tabla.bind("<Double-1>", self.elimina_datos)
    ...
    def elimina_datos(self, event):
        ...
        x = messagebox.askquestion('Informacion', '¿Deseas Borrar')
        if x=='yes':
                
                nombre = self.data['text']
                print(f"Nombre a eliminar:{nombre}")
                self.tabla.delete(item)
                self.base_datos.eliminar_datos(nombre)
    ...
    def agregar_datos(self):
        ...
        if nombre and edad and correo and telefono !='':
            ...
            self.base_datos.agregar_datos(nombre,edad,correo,telefono)
        ...
        for fila in datos:
            ...
            if nombre_bd == nombre:
                if Id != None:
                    ...
                    telefono = self.telefono.get()
                    if nombre and edad and correo and telefono !='':
                        self.base_datos.actulizar_datos(Id,nombre,edad,correo,telefono)
                        self.tabla.delete(*self.tabla.get_children())
                        datos = self.base_datos.mostrar_datos()
                        i= -1
                        for dato in datos:
...
if __name__ == "__main__":
    ...
    ventana.call('wm','iconphoto', ventana._w, PhotoImage(file='imagen/img.png'))
    app =Login(ventana)
    app.mainloop()
